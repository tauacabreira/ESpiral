function [final_waypoints] = ...
    write_waypoints_file_ecpp(file_name, waypoints, waypoints_info, home_coords, altitude)
    ...
    for i = 1 : max(waypoints_info)
        ...
        if i ~= max(waypoints_info) % 90 degree turns
            index_mid = index_end;
            index_ini = find(waypoints_info == line, 1, 'first');

            x = [waypoints(index_mid,1) waypoints(index_mid,2); 
                 waypoints(index_ini,1) waypoints(index_ini,2)];
