function[lineOfProjectedAreas, projectedArea, sideLines, firstCriticalLine, lastCriticalLine] = ...
    ...
    for k = 1 : numberOfWaypoints
        ...
        % the points of the side lines depend on the direction of the
        % coverage
        if(strcmp(direction,'clockwise'))
            pointA = [projectedArea(1,4) projectedArea(2,4)];
            pointB = [projectedArea(1,3) projectedArea(2,3)];
            pointC = [projectedArea(1,2) projectedArea(2,2)];
            pointD = [projectedArea(1,1) projectedArea(2,1)];
        else
            pointA = [projectedArea(1,2) projectedArea(2,2)];
            pointB = [projectedArea(1,1) projectedArea(2,1)];
